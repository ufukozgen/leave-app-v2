...
function CollapsibleSection({ title, count, defaultOpen = true, right, children }) {
    ...
    <div style={{ marginBottom: 8 }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <h2 style={{ fontFamily: "Urbanist, system-ui, sans-serif", fontWeight: 700, color: COLORS.grayDark, margin: 0, fontSize: 18 }}>
            {title}
          </h2>
          {count != null && (
            <span style={{ fontSize: 13, color: COLORS.gray, fontFamily: "Urbanist" }}>({count})</span>
          )}
          <span style={{ fontSize: 13, color: COLORS.gray }}>{open ? "▲" : "▼"}</span>
        </div>
...
function Pill({ children, tone = "info" }) {
  const map = { info: COLORS.blue, warn: COLORS.yellow, error: COLORS.red, ok: "#2e7d32" };
  return (
    <span style={{
      ...
      fontFamily: "Urbanist, system-ui, sans-serif", marginLeft: 8
    }}>{children}</span>
  );
}
...
export default function AdminBackups() {
  ...
  const [sortField, setSortField] = useState("created_at_ts");
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        if (le) throw le;
        setLogs((l ?? []).map(row => ({ ...row })));

        // Monthly usage: leaves with start_date in the selected month
        const { data: ur, error: ure } = await supabase
          .from("leave_requests")
          .select("user_id, leave_type_id, days, status, users(name, email)")
          .gte("start_date", startEnd.startD)
          .lt("start_date", startEnd.endD)
  ...
  // Group usage rows: user_id → { name, email, types: { leave_type_id: { deducted, approved } } }
  const usageGrouped = useMemo(() => {
    const grouped = {};
    usageRows.forEach(r => {
      const uid = r.user_id;
      if (!grouped[uid]) grouped[uid] = { name: r.users?.name ?? null, email: r.users?.email ?? null, types: {} };
      const tid = r.leave_type_id;
      if (!tid) return;
      if (!grouped[uid].types[tid]) grouped[uid].types[tid] = { deducted: 0, approved: 0 };
      if (r.status === "Deducted") grouped[uid].types[tid].deducted += r.days ?? 0;
      if (r.status === "Approved") grouped[uid].types[tid].approved += r.days ?? 0;
    });
    return grouped;
  }, [usageRows]);

  const usageSorted = useMemo(() => {
    return Object.entries(usageGrouped)
      .map(([user_id, { name, email, types }]) => ({ user_id, name, email, types }))
  ...
  const runBackupNow = async () => {
    ...
    try {
      ...
      if (!token) { toast.error("No session found. Please log in again."); return; }
      ...
      if (!res.ok || payload?.ok !== true) throw new Error(payload?.error || "Failed to trigger backup workflow");
  ...
  const exportMonthCSV = () => {
    const rows = sorted.map(r => ({
      ...
      balances: Object.entries(r.balances || {}).map(([k, v]) => `${leaveTypeName(k)}: ${v}`).join(" | "),
      approvals_not_deducted: Object.entries(r.approvals || {}).map(([k, v]) => `${leaveTypeName(k)}: ${v}`).join(" | "),
    }));
    downloadBlob(`leave-backups-${monthISO}.csv`, addUtf8Bom(jsonToCsv(rows)), "text/csv;charset=utf-8");
  };

  const exportMonthJSON = () => {
    downloadBlob(`leave-backups-${monthISO}.json`, JSON.stringify(sorted, null, 2), "application/json;charset=utf-8");
  };
  ...
  const exportOneCSV = (row) => {
    ...
    downloadBlob(`leave-backup-${row.user_email}-${row.snapshot_date}${timeSuffix}.csv`, addUtf8Bom(jsonToCsv(rows)), "text/csv;charset=utf-8");
  };

  const exportOneJSON = (row) => {
    const timeSuffix = row.run_ts ? "_" + new Date(row.run_ts).toISOString().slice(11, 19).replace(/:/g, "") : "";
    downloadBlob(`leave-backup-${row.user_email}-${row.snapshot_date}${timeSuffix}.json`, JSON.stringify(row, null, 2), "application/json;charset=utf-8");
  };

  const exportUsageCSV = () => {
    const rows = usageSorted.flatMap(({ name, email, types }) =>
      Object.entries(types).map(([tid, { deducted, approved }]) => ({
        user_name: name,
        user_email: email,
        leave_type: leaveTypeName(tid),
        deducted_days: deducted,
        approved_not_deducted: approved,
        total: deducted + approved,
      }))
    );
    downloadBlob(`monthly-usage-${monthISO}.csv`, addUtf8Bom(jsonToCsv(rows)), "text/csv;charset=utf-8");
  };

  return (
    <div style={{ padding: "16px" }}>
      {/* Top controls shared across all sections */}
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <input
          ...
          style={{ border: "1px solid #ddd", borderRadius: 8, padding: "6px 10px", fontFamily: "Urbanist, system-ui, sans-serif", minWidth: 200, flex: 1 }}
        />
        <button
          onClick={runBackupNow}
          disabled={dispatching}
          style={{
            padding: "8px 12px", borderRadius: 8, border: `1px solid ${COLORS.red}`,
            background: dispatching ? COLORS.veryLightBlue : "#fff", color: COLORS.grayDark,
            fontFamily: "Urbanist, system-ui", opacity: dispatching ? 0.7 : 1,
      ...
      {/* ── Leave Balance Backups ── */}
      <CollapsibleSection
        title="Leave Balance Backups"
        count={sorted.length || null}
        right={
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={exportMonthCSV} style={{ padding: "8px 12px", borderRadius: 8, border: `1px solid ${COLORS.orange}`, background: "#fff", color: COLORS.grayDark, fontFamily: "Urbanist, system-ui" }}>Export CSV</button>
            <button onClick={exportMonthJSON} style={{ padding: "8px 12px", borderRadius: 8, border: `1px solid ${COLORS.blue}`, background: "#fff", color: COLORS.grayDark, fontFamily: "Urbanist, system-ui" }}>Export JSON</button>
          </div>
        }
      >
      ...
      <CollapsibleSection
        title="Monthly Usage"
        count={usageSorted.length || null}
        right={
          usageSorted.length > 0 && (
            <button onClick={exportUsageCSV} style={{ padding: "8px 12px", borderRadius: 8, border: `1px solid ${COLORS.orange}`, background: "#fff", color: COLORS.grayDark, fontFamily: "Urbanist, system-ui" }}>
              Export CSV
            </button>
          )
        }
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              ...
              <button onClick={() => setSelected(null)} style={{ padding: "6px 10px", borderRadius: 8, border: `1px solid ${COLORS.gray}`, background: "#fff", fontFamily: "Urbanist" }}>Close</button>
            </div>
            ...
            <div style={{ background: COLORS.veryLightBlue, padding: 12, borderRadius: 8, fontFamily: "Calibri, system-ui" }}>
              {Object.entries(selected.balances || {}).map(([k, v]) => <div key={k} style={{ marginBottom: 6 }}><strong>{leaveTypeName(k)}</strong>: {v}</div>)}
            ...
            <div style={{ background: "#fff", border: `1px dashed ${COLORS.lightBlue}`, padding: 12, borderRadius: 8, fontFamily: "Calibri, system-ui" }}>
              {Object.entries(selected.approvals || {}).map(([k, v]) => <div key={k} style={{ marginBottom: 6 }}><strong>{leaveTypeName(k)}</strong>: {v}</div>)}
              {(!selected.approvals || Object.keys(selected.approvals).length === 0) && <em>(None)</em>}
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <button onClick={() => exportOneCSV(selected)} style={{ padding: "8px 12px", borderRadius: 8, border: `1px solid ${COLORS.orange}`, background: "#fff", fontFamily: "Urbanist" }}>Export CSV</button>
              <button onClick={() => exportOneJSON(selected)} style={{ padding: "8px 12px", borderRadius: 8, border: `1px solid ${COLORS.blue}`, background: "#fff", fontFamily: "Urbanist" }}>Export JSON</button>
            </div>
          </div>
        </div>
      )}
